...
summary(res)
```
First, we fitted a linear regression model using the lm() function. The model predicts civilian_complaints_sustained_2020 (dependent variable) based on Total_Less_lethal_force_2020 (independent variable). Then, we calculated the residuals of the linear regression model. We created a plot that is used to check for non-random patterns in the residuals, which can indicate issues with the model (like non-linearity or heteroscedasticity). Next, we generated a Quantile-Quantile (Q-Q) plot of the residuals. It’s used to assess whether the residuals are normally distributed. Additionally, we created a density plot of the residuals, another way to visually assess the normality of residuals. The symmetric, bell-shaped curve centered around zero indicates normality. The primary purpose of these steps is to assess the fit and assumptions of the linear regression model. The mean of the residuals being 0 is a good sign, indicating no bias in prediction. However, the wide range between the minimum and maximum values suggests some predictions are far off the mark. The negative values for the first three quartiles (Min, 1st Quartile, Median) suggests a tendency of the model to overestimate. The significiant amount of negative and positive residuals may indicate problems such as non-linearity or influential outliers. 


Indication of Overestimation: The negative values for the first three quartiles (Min, 1st Qu., Median) suggest a tendency of the model to overestimate.

Presence of Outliers or Extreme Values: The large difference between the minimum and maximum residuals implies the presence of outliers or extreme values in the data that are not well accounted for by the model.

Potential Model Issues: The substantial negative and positive residuals might indicate problems such as non-linearity, heteroscedasticity (non-constant variance of residuals), or influential outliers.
...
##  Limtations⛔️
 From many visualizations like the heatmap and diversity bar charts, we see a positive correlation between police force diversity and civilian complaints (our target variables), a finding that seems counterintuitive to our hypothesis.This insight propels us to investigate further: we are now exploring potential confounders or other underlying factors that may influence this relationship. Additionally, although there is an absence of a clear pattern in our Pearson residual plot, it appears we may have encountered overdispersion as shown in our deviance residual plot. In the next step, we can use the negative binominal regression model to adjust for this.
