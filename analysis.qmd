...
```
##  Limtations:
Although there is an relationship between the two target variables, they are positively correlated, which is different from our common sense. That will lead us to figure out is there any confounders related to the target variables. What's more, according to the residual plot, it seems like we encountered overdispersion. In the next step, we can use the negative binominal regression model to fix it.
