...
ori_frequency_df1 <- table(cleansed_df1$ORI)
values_with_frequency_gt_1 <- names(ori_frequency_df1[ori_frequency_df1 = 1])
cleansed_df1 <- cleansed_df1[cleansed_df1$ORI %in% values_with_frequency_gt_1, ]
print(cleansed_df1)

ori_frequency_use <- table(use_for_force$ORI)
values_with_frequency_gt_1 <- names(ori_frequency_use[ori_frequency_use = 1])
use_for_force <- use_for_force[use_for_force$ORI %in% values_with_frequency_gt_1, ]
print(filtered_data)
```

```{r}
table(use_for_force$ORI)
colnames(use_for_force)[colnames(use_for_force) == "Police Department"] <- "police_department"
use_for_force
cleansed_df1
merged_data <- merge(use_for_force,cleansed_df1, by = "police_department", all = TRUE)
head(merged_data)
```
